...
public class OutboxEventFactory {
    ...
    public OutboxItem create(OrderCreatedEvent event) {

        try {

            String now = event.occurredAt().toString();

            OutboxItem item = new OutboxItem();

            item.setPk("OUTBOX#" + event.eventId());
            item.setSk("OUTBOX#" + event.eventId());

            item.setGsi1pk("OUTBOX#PENDING");
